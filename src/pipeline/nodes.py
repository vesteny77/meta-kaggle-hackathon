...
def csv_to_parquet(src_dir: Path, dst_dir: Path, tables: dict) -> dict:
    ...
    try:
        ...
        print(f"Available memory: {available_memory / (1024*1024*1024):.2f} GB, using chunk size: {CHUNK_SIZE:,} rows")
    except ImportError:
        # If psutil isn't available, use default chunk size
        CHUNK_SIZE = DEFAULT_CHUNK_SIZE
        print(f"Using default chunk size: {CHUNK_SIZE:,} rows (install psutil for adaptive sizing)")
    except Exception as e:
        # If anything goes wrong, fall back to default
        CHUNK_SIZE = DEFAULT_CHUNK_SIZE
        print(f"Error determining memory: {str(e)}. Using default chunk size: {CHUNK_SIZE:,} rows")
    ...
    for tbl, date_col in tables.items():
        try:
            ...
            if use_chunking:
                print(f"Large file detected for {tbl} ({file_size / (1024*1024*1024):.2f} GB). Using chunked processing.")
        except Exception as e:
                    ...
                    if date_col in sample_df.columns:
                        ...
                        if not sample_df[date_col].is_empty():
                            for fmt in date_formats:
                                try:
                                    ...
                                    test_df = sample_df.with_columns([
                                        pl.col(date_col).str.strptime(pl.Datetime, fmt).alias(f"{date_col}_parsed")
                                    ])
                                    # If we get here, the format works
                                    success = True
                                    fmt_to_use = fmt
                                    print(f"\u2713 Using date format '{fmt}' for column '{date_col}'")
                                    break
                                except Exception as e:
                            ...
                            if not success:
                                ...
                                print(f"Warning: Could not parse date column '{date_col}' in {tbl}. Keeping as string.", file=sys.stderr)
                                print(f"Attempted formats: {', '.join(date_formats)}", file=sys.stderr)
                                print(f"Errors: {'; '.join(error_messages)}", file=sys.stderr)
                        else:
                            print(f"Warning: Date column '{date_col}' is empty in {tbl}", file=sys.stderr)
                    else:
                        ...
                        if date_col.lower() in columns_lower:
                            ...
                            actual_col = sample_df.columns[columns_lower.index(date_col.lower())]
                            print(f"Warning: Column name case mismatch in {tbl}. Expected '{date_col}', found '{actual_col}'.", file=sys.stderr)
                            print(f"Please update your configuration to use the correct case: '{actual_col}'", file=sys.stderr)
                            date_col = actual_col
                            ...
                            print(f"Warning: Date column '{date_col}' not found in {tbl}", file=sys.stderr)
                            print(f"Available columns: {', '.join(sample_df.columns)}", file=sys.stderr)
                except Exception as e:
                    print(f"Warning: Error analyzing sample data for {tbl}: {str(e)}", file=sys.stderr)
            ...
            if use_chunking:
                ...
                if date_col and success and fmt_to_use:
                    first_chunk_scan = first_chunk_scan.with_columns([
                        pl.col(date_col).str.strptime(pl.Datetime, fmt_to_use).alias(date_col)
                    ])
                
                # Collect and write the first chunk with write_mode="overwrite"
                first_chunk = first_chunk_scan.collect()
                try:
                    first_chunk.write_parquet(output_path, compression="zstd", **writer_opts)
                    chunk_time = time.time() - chunk_start
                    print(f"\u2713 Wrote first chunk ({len(first_chunk)} rows) in {chunk_time:.2f} seconds")
                except Exception as e:
                    if "already exists" in str(e).lower() or "not a directory" in str(e).lower() or "os error 20" in str(e).lower():
                        ...
                        first_chunk.write_parquet(output_path, compression="zstd", **writer_opts)
                        chunk_time = time.time() - chunk_start
                        print(f"\u2713 Cleaned up path and wrote first chunk ({len(first_chunk)} rows) in {chunk_time:.2f} seconds")
                    else:
                ...
                estimated_total_rows = int(file_size / (offset / len(first_chunk))) if len(first_chunk) > 0 else 0
                estimated_chunks = estimated_total_rows // CHUNK_SIZE + 1
                print(f"Estimated total: ~{estimated_total_rows:,} rows in ~{estimated_chunks} chunks")
                ...
                while True:
                    ...
                    while retry_count < MAX_RETRIES and not success:
                        try:
                            ...
                            if retry_count > 0:
                                ...
                                current_chunk_size = max(1000, CHUNK_SIZE // (2 ** retry_count))
                            ...
                            try:
                                chunk.write_parquet(output_path, compression="zstd", mode="append", **writer_opts)
                                chunk_time = time.time() - chunk_start
                                success = True
                            except Exception as append_error:
                                if "already exists" in str(append_error).lower() or "not a directory" in str(append_error).lower() or "os error 20" in str(append_error).lower():
                                    print(f"\n\u2717 Error appending to parquet file: {str(append_error)}", file=sys.stderr)
                                    ...
                                    partition_dirs = [p for p in glob.glob(f"{output_path}/*/") if not Path(p).is_dir()]
                                    for problem_path in partition_dirs:
                                        print(f"Removing problematic path: {problem_path}")
                                    ...
                                    chunk_time = time.time() - chunk_start
                                    success = True
                                    print("\u2713 Fixed directory structure and appended chunk successfully")
                                else:
                            ...
                            if retry_count >= MAX_RETRIES:
                                print(f"\n\u2717 Failed to process chunk after {MAX_RETRIES} attempts: {str(e)}", file=sys.stderr)
                                # Try to continue with the next chunk
                                offset += current_chunk_size
                                print(f"Skipping to next chunk at offset {offset}")
                                break
                            else:
                                print(f"\n\u2717 Error processing chunk: {str(e)}. Retrying...", file=sys.stderr)
                    ...
                    progress_pct = min(100, (chunk_num / estimated_chunks) * 100) if estimated_chunks > 0 else 0
                    ...
                    est_total_time = estimated_total_rows / rows_per_sec if rows_per_sec > 0 else 0
                    ...
                    print(f"\rChunk {chunk_num}/{estimated_chunks} | Progress: [{bar}] {progress_pct:.1f}% | " 
                          f"Rows: {total_rows:,}/{estimated_total_rows:,} | "
                          f"Speed: {rows_per_sec:.1f} rows/s | "
                          f"Elapsed: {format_time(elapsed_time)} | "
                          f"Est. remaining: {format_time(est_remaining)}", end="")
                ...
                try:
                    scan.collect().write_parquet(output_path, compression="zstd", **writer_opts)
                except Exception as e:
                    if "already exists" in str(e).lower() or "not a directory" in str(e).lower() or "os error 20" in str(e).lower():
                        # Clean up existing file/directory
                        import shutil
...
def build_bigjoin(parquet_files: dict, output_dir: Path) -> Path:
    ...
    required_tables = ['KernelVersions', 'Competitions', 'KernelVersionCompetitionSources', 'KernelVersionDatasetSources']
    ...
    # Register Parquet scans
    for name, path in parquet_files.items():
        table_alias = name[0].lower()
        try:
            con.register(table_alias, pl.scan_parquet(path))
            print(f"✓ Registered {name} as {table_alias}")
        except Exception as e:
            print(f"✗ Failed to register {name}: {str(e)}", file=sys.stderr)
            raise
    
    # Register specific tables with more descriptive aliases
    try:
        if 'KernelVersionCompetitionSources' in parquet_files:
            con.register('kvcs', pl.scan_parquet(parquet_files['KernelVersionCompetitionSources']))
            print("✓ Registered KernelVersionCompetitionSources as kvcs")
        
        if 'KernelVersionDatasetSources' in parquet_files:
            con.register('kvds', pl.scan_parquet(parquet_files['KernelVersionDatasetSources']))
            print("✓ Registered KernelVersionDatasetSources as kvds")
    except Exception as e:
        print(f"✗ Failed to register relationship tables: {str(e)}", file=sys.stderr)
        raise
    
    # Validate table registrations
    try:
        tables_query = "SELECT * FROM information_schema.tables WHERE table_schema = 'main'"
        registered_tables = con.sql(tables_query).collect().to_pandas()
        print(f"Registered tables: {', '.join(registered_tables['table_name'].tolist())}")
    except Exception as e:
        print(f"Warning: Unable to list registered tables: {str(e)}", file=sys.stderr)
    ...
    SELECT 
        ...
        k.TotalTokens      AS totalTokens,
        k.ExecSeconds      AS execSeconds,
        k.GpuType          AS gpuType,
        k.IsCommit         AS isCommit,
        c.Id               AS comp_id,
        c.Title            AS comp_title,
        c.Category         AS category,
        c.DeadlineDate     AS endDate,
    ...
    """
    try:
        # Before executing the main query, try to inspect each table
        print("\nVerifying table schemas before running the main query:")
        try:
            print("Table 'k' schema:")
            schema_k = con.sql("SELECT * FROM k LIMIT 0").schema
            for col_name, col_type in zip(schema_k.names, schema_k.types):
                print(f"  - {col_name}: {col_type}")
        except Exception as e:
            print(f"\u2717 Error inspecting table 'k': {str(e)}", file=sys.stderr)
            
        try:
            print("\nTable 'kvcs' schema:")
            schema_kvcs = con.sql("SELECT * FROM kvcs LIMIT 0").schema
            for col_name, col_type in zip(schema_kvcs.names, schema_kvcs.types):
                print(f"  - {col_name}: {col_type}")
        except Exception as e:
            print(f"\u2717 Error inspecting table 'kvcs': {str(e)}", file=sys.stderr)
            
        try:
            print("\nTable 'c' schema:")
            schema_c = con.sql("SELECT * FROM c LIMIT 0").schema
            for col_name, col_type in zip(schema_c.names, schema_c.types):
                print(f"  - {col_name}: {col_type}")
        except Exception as e:
            print(f"\u2717 Error inspecting table 'c': {str(e)}", file=sys.stderr)
            
        try:
            print("\nTable 'kvds' schema:")
            schema_kvds = con.sql("SELECT * FROM kvds LIMIT 0").schema
            for col_name, col_type in zip(schema_kvds.names, schema_kvds.types):
                print(f"  - {col_name}: {col_type}")
        except Exception as e:
            print(f"\u2717 Error inspecting table 'kvds': {str(e)}", file=sys.stderr)
            
        # Execute the main query
        print("\nExecuting main SQL query to build bigjoin...")
        con.sql(sql)
        print("\u2713 SQL query executed successfully")
        
        # Verify the result table was created
        try:
            result_count = con.sql("SELECT COUNT(*) FROM kernel_bigjoin").fetchone()[0]
            print(f"\u2713 Created kernel_bigjoin with {result_count} rows")
            
            # Show a sample of the data
            print("\nSample of kernel_bigjoin (first 5 rows):")
            sample = con.sql("SELECT * FROM kernel_bigjoin LIMIT 5").df()
            print(sample)
        except Exception as e:
            print(f"\u2717 Error verifying result table: {str(e)}", file=sys.stderr)
    except Exception as e:
        print(f"\n\u2717 Failed to execute SQL query: {str(e)}", file=sys.stderr)
        
        # Try to diagnose the issue
        error_msg = str(e).lower()
        if "table with name" in error_msg and "does not exist" in error_msg:
            table_name = error_msg.split("table with name")[1].split("does not exist")[0].strip()
            print(f"\nDiagnosis: Missing table '{table_name}'")
            ...
            import re
            column_match = re.search(r"column\s+([\w.]+)\s+does not exist", error_msg)
            if column_match:
                column_name = column_match.group(1)
                print(f"\nDiagnosis: Missing column '{column_name}'")
                print("This could be due to:")
                print("1. The column name case is incorrect in the SQL query")
                print("2. The column doesn't exist in the source table")
                print("3. There's a typo in the column name")
    ...
    try:
        con.sql(export_sql)
        print(f"\n✓ Successfully exported bigjoin to {output_path}")
    except Exception as e:
        print(f"\n✗ Failed to export bigjoin: {str(e)}", file=sys.stderr)
        # Check if the table was created successfully before failing on export
        try:
            row_count = con.sql("SELECT COUNT(*) FROM kernel_bigjoin").fetchone()[0]
            print(f"Note: kernel_bigjoin table contains {row_count} rows")
        except Exception as inner_e:
            print(f"Additionally, couldn't query the table: {str(inner_e)}", file=sys.stderr)
        raise
    
    # Check the output file was created
    if not output_path.exists():
        print(f"\n✗ Warning: Output file {output_path} does not exist after export", file=sys.stderr)
...
def create_mini_meta(input_path: Path, output_path: Path, sample_frac: float = 0.01) -> Path:
    """
