...
def test_mini_meta_consistency(data_dir):
    ...
    mini_meta_count = (
        pl.scan_parquet(mini_meta_path).select(pl.count()).collect().item()
    )
    full_data_count = (
        pl.scan_parquet(full_data_path).select(pl.count()).collect().item()
    )

    # Allow for some variation due to hash-based sampling
    assert (
        0.005 <= mini_meta_count / full_data_count <= 0.015
