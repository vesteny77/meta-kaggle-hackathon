...
def process_kernel(kernel_id: int, meta_row: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """
    ...
    features.update({
        ...
        "uses_gpu": meta_row.get("gpuType") is not None and meta_row.get("gpuType") != "None",
    })
...
def main():
    ...
    parser.add_argument("--limit", type=int, default=0, help="Limit the number of kernels to process (0 for all)")
    ...
    # Process kernels
    features = []
    kernel_count = len(meta_df)
    print(f"Processing {kernel_count} kernels...")
    
    for i, row in enumerate(meta_df.iter_rows(named=True)):
        if i % 100 == 0:
            print(f"Progress: {i}/{kernel_count} ({i/kernel_count:.1%})")
            
        kernel_id = row["kernel_id"]
        features_dict = process_kernel(kernel_id, row)
    ...
    features_df.write_parquet(output_path, compression="zstd")
    print(f"Processed {len(features)} kernels successfully")
